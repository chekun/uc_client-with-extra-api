...
class usermodel {
	...
	function rename_user($old_username, $new_username)
	{
		...
		{
			//查找老用户
			$user = $this->db->fetch_first("SELECT username, uid FROM ".UC_DBTABLEPRE."members WHERE username='$old_username'");
			if ($user)
			{
				//查找是否新用户名有重复的
				$is_existed = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."members WHERE username='$new_username' AND uid!='$user[uid]'");
				if ( ! $is_existed)
				{
					$this->db->query("UPDATE ".UC_DBTABLEPRE."members SET username='$new_username' WHERE uid='$user[uid]'");
					if ($this->db->affected_rows() === 1)
					{
						$this->base->load('note');
						$_ENV['note']->add('renameuser', 'uid='.$user['uid'].'&oldusername='.urlencode($old_username).'&newusername='.urlencode($new_username));
						return 1;//修改成功
					}
					return 0;//未做修改
				}
